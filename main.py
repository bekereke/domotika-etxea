...
I2C_LCD1602.clear()
basic.pause(100)
I2C_LCD1602.show_string("Sartu pasahitza", 0, 0)
pins.digital_write_pin(DigitalPin.P16, 0)
...
def on_forever():
    ...
    if temp_flag == 1:
        for index in range(1):
            I2C_LCD1602.show_string("tenperatura", 0, 0)
        I2C_LCD1602.show_number(input.temperature(), 0, 1)
        basic.pause(500)
    elif water_flag == 1:
        for index2 in range(1):
            I2C_LCD1602.show_string("kea", 0, 0)
        ...
        if pins.digital_read_pin(DigitalPin.P1) == 0:
            if dangerous_flag == 0:
                I2C_LCD1602.clear()
                I2C_LCD1602.show_string("arriskua", 0, 0)
                basic.pause(500)
                dangerous_flag = 1
                security_flag = 0
        else:
            if security_flag == 0:
                I2C_LCD1602.clear()
                I2C_LCD1602.show_string("segurtasuna", 0, 0)
                basic.pause(500)
                dangerous_flag = 0
                security_flag = 1
    elif people_flag == 1:
        if pins.digital_read_pin(DigitalPin.P15) == 1:
            if someone_flag == 0:
                I2C_LCD1602.clear()
                I2C_LCD1602.show_string("norbait dago", 0, 0)
                basic.pause(500)
                someone_flag = 1
        else:
            if someone_flag == 1:
                I2C_LCD1602.clear()
                I2C_LCD1602.show_string("inor ez dago", 0, 0)
                basic.pause(500)
                someone_flag = 0
