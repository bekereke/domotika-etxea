...
input.onButtonPressed(Button.AB, function () {
    ...
    if (passwd_enter == password) {
        I2C_LCD1602.ShowString("Arrakasta", 0, 0)
        I2C_LCD1602.ShowString("Ireki atea", 0, 1)
        pins.servoWritePin(AnalogPin.P8, 180)
        passwd_enter = ""
    } else {
        I2C_LCD1602.ShowString("Berriro sartu", 0, 0)
        I2C_LCD1602.ShowString("Akatsa", 0, 1)
        passwd_enter = ""
        basic.pause(1000)
        I2C_LCD1602.clear()
        I2C_LCD1602.ShowString("Sartu pasahitza", 0, 0)
    }
})
...
basic.forever(function () {
    ...
    if (temp_flag == 1) {
        for (let index = 0; index < 1; index++) {
            I2C_LCD1602.ShowString("tenperatura", 0, 0)
        }
        I2C_LCD1602.ShowNumber(input.temperature(), 0, 1)
        basic.pause(500)
    } else if (water_flag == 1) {
        for (let index = 0; index < 1; index++) {
            I2C_LCD1602.ShowString("kea", 0, 0)
        }
        ...
        if (pins.digitalReadPin(DigitalPin.P1) == 0) {
            if (dangerous_flag == 0) {
                I2C_LCD1602.clear()
                I2C_LCD1602.ShowString("arriskua", 0, 0)
                basic.pause(500)
                dangerous_flag = 1
                security_flag = 0
            }
            ...
            if (security_flag == 0) {
                I2C_LCD1602.clear()
                I2C_LCD1602.ShowString("segurtasuna", 0, 0)
                basic.pause(500)
                dangerous_flag = 0
                security_flag = 1
            }
        ...
        if (pins.digitalReadPin(DigitalPin.P15) == 1) {
            if (someone_flag == 0) {
                I2C_LCD1602.clear()
                I2C_LCD1602.ShowString("norbait dago", 0, 0)
                basic.pause(500)
                someone_flag = 1
            }
        } else {
            if (someone_flag == 1) {
                I2C_LCD1602.clear()
                I2C_LCD1602.ShowString("inor ez dago", 0, 0)
                basic.pause(500)
                someone_flag = 0
            }
        }
